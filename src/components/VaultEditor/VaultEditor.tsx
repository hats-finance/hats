...
export default function VaultEditor() {
  ...
  const [vaultDescription, setVaultDescription] =
    useState<IVaultDescription>(newVaultDescription);
  ...
  async function loadFromIpfs(ipfsHash) {
    try {
      ...
      if ("Project-metadata" in newVaultDescription) {
        newVaultDescription["project-metadata"] =
          newVaultDescription["Project-metadata"];
  ...
  function addMember() {
    setVaultDescription((prev) => {
      let newObject = { ...prev };
      setPath(newObject, "committee.members", [
  ...
  function addPgpKey(pgpKey) {
    setVaultDescription((prev) => {
      ...
      const sureArray =
        typeof keys === "string" ? (keys === "" ? [] : [keys]) : keys;
      setPath(newObject, "communication-channel.pgp-pk", [
        ...sureArray,
        pgpKey
      ]);
      return newObject;
  ...
  function removePgpKey(index: number) {
    ...
    if (typeof value === "string") {
      ...
      let newVaultDescription = removeFromArray(
        vaultDescription,
        "communication-channel.pgp-pk",
        index
      );
      setVaultDescription(newVaultDescription);
    }
  ...
  function removeContract(index: number) {
    let newContracts = removeFromArray(
      contracts,
      "contracts",
      index,
      newContract
  ...
  function severitiesToContracts(vaultDescription: IVaultDescription) {
    ...
    vaultDescription.severities.forEach((severity) => {
      const contractsCovered =
        severity["contracts-covered"]?.length === 0
          ? [newContract]
          : severity["contracts-covered"];
          ...
          <div className="vault-editor__section">
            ...
            <div className="vault-editor__section-content">
              ...
              <EditableContent
                ...
                onChange={onChange}
                placeholder={t(
                  "VaultEditor.vault-details.multisig-address-placeholder"
                )}
              />
            </div>
          </div>
          ...
          <div className="vault-editor__section">
            ...
            <div className="vault-editor__section-content">
              <div className="committee-members">
                {(vaultDescription?.committee?.members || []).map(
                  (member, index) => (
                    <CommmitteeMember
                      key={index}
                      member={member}
                      index={index}
                      membersCount={
                        (vaultDescription?.committee?.members || []).length
                      }
                      onChange={onChange}
              ...
              <div className="contracts-covered">
                {(contracts.contracts || []).map((contract, index) => (
                  <ContractCovered
                    key={index}
                    contract={contract}
                    severitiesOptions={vaultDescription.severities.map(
                      (severity) => ({
                        label: severity.name,
                        value: severity.name
                      })
            ...
            <div className="vault-editor__section-content">
              <VaultProvider>
                <CommunicationChannel
                  ...
                  communicationChannel={
                    vaultDescription?.["communication-channel"]
                  }
                  addPgpKey={addPgpKey}
                  onChange={onChange}
                />

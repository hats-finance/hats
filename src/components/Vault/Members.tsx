  ...
  const members = props.members?.map((member: ICommitteeMember, index: number) => {
    return (
      <a className="member-link-wrapper" key={index} href={member?.["twitter-link"]} target="_blank" rel="noreferrer">
        {member?.["image-ipfs-link"] ? <img src={props.preview ? member?.["image-ipfs-link"] : `${IPFS_PREFIX}${verifyIPFSLink(member?.["image-ipfs-link"])}`} alt="twitter avatar" className="twitter-avatar" /> : <TwitterImageIcon />}

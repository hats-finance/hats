...
interface FormJSONCSVFileInputProps {
...
function FormJSONCSVFileInputComponent(
  { onChange, colorable = false, isDirty = false, label, error, disabled = false, ...props }: FormJSONCSVFileInputProps,
  ref
) {
  ...
  return (
    <StyledFormJSONCSVFileInput isDirty={parseIsDirty(isDirty) && colorable} hasError={!!error && colorable} disabled={disabled}>
      <input
        {...props}
        disabled={disabled}
        type="hidden"
